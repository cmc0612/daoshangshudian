...
import org.apache.commons.dbutils.QueryRunner;//还有一个核心类是ResultSetHandler
...
public class ProductDao {
	...
	public void addProduct(Product p) throws SQLException {
		...
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());//以DataSource作为参数以获取连接对象。
		runner.update(sql, p.getId(), p.getName(), p.getPrice(),
				p.getCategory(), p.getPnum(), p.getImgurl(), p.getDescription());//该方法可以执行更新插入
	}
	// 查找所有商品
